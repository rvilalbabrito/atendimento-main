...
import { checkerExpedientQueue } from "../../helpers/checkerExpedient";
...
const verifyQueue = async (
  ...
  if (choosenQueue) {
    const checker = await checkerExpedientQueue(choosenQueue.id);

    if (!checker) {
      if (
        !choosenQueue.outOfWorkMessage ||
        choosenQueue.outOfWorkMessage.trim() === ""
      ) {
        return;
      }
      ...
      const sentMessage = await wbot.sendMessage(
        ...
        {
          text: choosenQueue.outOfWorkMessage
        }
      );
